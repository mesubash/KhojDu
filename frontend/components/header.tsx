...
export function Header({ hideNavigation = false }: HeaderProps) {
  ...
  const userInfo = user
    ? {
        ...
        email: user.email,
        avatar: user.avatar,
        initials: user.fullName ? user.fullName.split(" ").map((n) => n[0]).join("").toUpperCase() : user.email.charAt(0).toUpperCase(),
      }
    : undefined
  ...
  const isActiveRoute = (href: string) => {
    ...
    if (href.startsWith("/dashboard")) return pathname === href || pathname.startsWith("/dashboard")
              ...
              <nav className="hidden md:flex items-center space-x-1">
                {navigation.map((item) => (
                  <Link
                    key={item.name}
                    href={item.href}
                    className={`relative px-3 py-2 text-sm font-medium transition-all duration-300 hover:text-orange-600 group ${isActiveRoute(item.href) ? "text-orange-600" : "text-muted-foreground"}`}
                  >
                    {item.name}
                    <span
                      className={`absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-orange-500 to-orange-600 transform transition-all duration-300 ${isActiveRoute(item.href) ? "scale-x-100 opacity-100" : "scale-x-0 opacity-0 group-hover:scale-x-100 group-hover:opacity-70"}`}
                    />
                  </Link>
                ))}
                <a
                    ...
                    <DropdownMenuContent className="w-56" align="end" forceMount>
                      <div className="flex items-center justify-start gap-2 p-2">
                        <div className="flex flex-col space-y-1 leading-none">
                          ...
                          {userInfo?.email && <p className="w-[200px] truncate text-sm text-muted-foreground">{userInfo.email}</p>}
                        </div>
                      </div>
                      <DropdownMenuSeparator />
              ...
              <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-background border rounded-lg mt-2">
                {navigation.map((item) => (
                  <Link
                    key={item.name}
                    href={item.href}
                    className={`block px-3 py-2 text-base font-medium transition-colors rounded-md ${isActiveRoute(item.href) ? "text-orange-600 bg-orange-50 dark:bg-orange-950/20" : "text-muted-foreground hover:text-orange-600 hover:bg-muted"}`}
                ...
                <div className="border-t pt-4 mt-4">
                  {isAuthenticated ? (
                    <>
                      ...
                      <Button variant="ghost" className="w-full justify-start mt-2 text-red-600 hover:text-red-700" onClick={handleLogout}>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl border border-border max-w-sm w-full p-6 space-y-4">
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground">You will need to sign in again to access your dashboard.</p>
              </div>
            </div>
